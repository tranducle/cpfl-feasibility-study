...
## Figure — Feasibility gate matrix (firms × layers)

| Field | Value |
|---|---|
| File | `figures/fig_gate_matrix.pdf`; source `code/fig_gate_matrix.tex` |
| Script | Hand-authored TikZ (`standalone`); not generated by `generate_figures.py` |
| Data source | Derived from `tab:firm-gate` in the manuscript (§5.1); no values invented |
| Plot type | Pass/fail heatmap matrix (firms × {G^E, G^P, G^C, G^R} + layer-complete) |
| Encoding | ✓ pass (green), ✗ fail (rose), 3/4 feasibility-counted (amber); grayscale-safe via glyphs |
| Evidence claim | Event passes 6/6; human coding 0/6; only Mr. Cooper reaches 3 layers (censored) → BOUND |
| Claim discipline | Methodology/feasibility figure (not an empirical-result figure); no coefficient, no fitted model |
| Manuscript section | §5.1 Gate results (alongside `tab:firm-gate`) |
| Build | `pdflatex code/fig_gate_matrix.tex` (palette matches Figs 1–4) |
